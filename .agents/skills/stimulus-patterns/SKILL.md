...
description: >-
  ...
  server-side architecture decisions (see rails-architecture skill).
license: MIT
...
**Tech Stack:** Stimulus, Turbo, Rails-rendered views, and the project CSS system
...
```ruby
# Prefer request/model specs for backend behavior. Add system specs only for UI behavior
# that cannot be covered through cheaper tests.
RSpec.describe "card details", type: :system do
  it "toggles details" do
    visit card_path(card)
    expect(page).to have_no_css(".card__details")
    click_button "Show Details"
    expect(page).to have_css(".card__details")
  end
end
```
